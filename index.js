...
// Terminal se input lene ke liye basic setup (agar number config me na ho)
...
async function startBot() {
    ...
    const sock = makeWASocket({
        ...
        auth: state,
        browser: ["Ubuntu", "Chrome", "20.0.04"] // Pairing code ke liye zaroori hai
    });

    // PAIRING CODE LOGIC: Agar pehle se login nahi hai toh code generate karega
    if (!sock.authState.creds.registered) {
        let phoneNumber = config.ownerNumber;
        
        // Agar config me number galat ya default hai toh terminal poochay ga
        if (!phoneNumber || phoneNumber.includes('X')) {
            phoneNumber = await question('👉 Apna WhatsApp Number enter karein (with Country Code, e.g., 923xxxxxxxx):\n');
        }
        
        phoneNumber = phoneNumber.replace(/[^0-9]/g, ''); // Sirf numbers rkhne k lya
        ...
        setTimeout(async () => {
            try {
                ...
                console.log('👉 Is code ko apne WhatsApp -> Linked Devices -> Link with Phone Number me daalein.');
                console.log('\n──────────────────────────────────────────\n');
            } catch (error) {
                console.log('Pairing code request fail ho gayi: ', error);
            }
        }, 3000); // 3 seconds ka delay taaki connection stable ho
    }
    ...
    sock.ev.on('connection.update', (update) => {
        ...
        if (connection === 'close') {
            ...
            if (shouldReconnect) {
                ...
                console.log('❌ Session logged out. Please delete auth_info_baileys folder and restart.');
            }
